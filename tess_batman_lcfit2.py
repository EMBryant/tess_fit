...
def lc_min(params, phase, flux):
	...
	residuals = flux - f_model
	return residuals

def lc_bin_qlp(time, flux, bin_width):
	'''
...
def lc_fit_spoc(time, flux, err, period, epoch, tdur, objid, Tmag, Rs):
    '''
    ...
    res = minimize(lc_min, params, args=(p_fit, f_fit), method='leastsq') #perform minimization
    fit_val = np.mean(res.residual)
    ...
    print('Minimization result: {}: {}; Residual ave = {:.4f}'.format(res.success, res.message, fit_val))
    ...
    plt.xlabel('Phase')
    plt.ylabel('Flux')
    plt.title('TIC  {}; T = {:.3f};  Rs = {:.3f} \n Depth: {:.5f}%;  Duration: {:5f} hours; Epoch; {:.6f} + {:.8f} \n (Rp/Rs): {:.6f};  Res ave: {:.8f}; Period: {:.6f} days'.format(objid, Tmag, Rs, t_depth, t_dur, epoch, t0_best, rp_best, fit_val, period))
...
def lc_fit_spoc_lk(time, flux, flux_unflat, err, period, epoch, tdur, objid):
    '''
    Function to fit a batman model to an input lc datafile to find the best 
    fit system parameters
    '''
    ...
    f_fit = flux[np.abs(phase) < 0.2]   #in range (-0.2 ,  0.2)
    e_fit = err[np.abs(phase) < 0.2]
    f2_fit = flux_unflat[np.abs(phase) < 0.2]
    ...
    plt.xlabel('Phase')
    plt.ylabel('Flux')
    plt.title('Depth: {:.5f}%;  Duration: {:5f} hours; Epoch; {:.6f} + {:.8f} \n (Rp/Rs): {:.6f};  res ave: {:.8f}; Period: {:.6f} days'.format(t_depth, t_dur, epoch, t0_best, rp_best, fit_val, period))
    if np.isnan(t_dur) == False:
        plt.xlim((-3*p4, 3*p4))
        plt.ylim((f_bin.min()-t_depth/200, 1+t_depth/200))
...
def lc_fit_qlp(time, flux, period, epoch, tdur, objid):
    '''
    Function to fit a batman model to an input lc datafile to find the best 
    fit system parameters
    '''
    ...
    #Produce binned data set for plotting
    bw = 10 / (1440*period)                    #Bin width - 10 mins in units of phase
    p_bin, f_bin, e_bin = lc_bin_qlp(p_fit, f_fit, bw)
    
    #Produce plot of data and best fit model LC
    plt.figure(figsize=(9, 7.5))
    
    plt.plot(p_fit, f_fit, marker='o', color='gray', linestyle='none', markersize=3)
    plt.errorbar(p_bin, f_bin, yerr=e_bin, marker='o', linestyle='none', color='red', markersize=5)
    plt.plot(p_best, f_best, 'g--', linewidth=2)
    
    plt.xlabel('Phase')
    plt.ylabel('Flux')
    plt.title('TIC  {}; Depth: {:.5f}%;  Duration: {:5f} hours \n Epoch; {:.6f} + {:.8f}; Period: {:.6f} days \n (Rp/Rs): {:.6f};  Res ave: {:.8f}'.format(objid, t_depth, t_dur, epoch, t0_best, period, rp_best, fit_val))
    if np.isnan(t_dur) == False:
        plt.xlim((-3*p4, 3*p4))
        plt.ylim((f_bin.min()-t_depth/200, 1+t_depth/200))
    ...
    plt.show()
    return p_best, f_best, p_fit, f_fit, rp_best, a_best, inc_best, t0_best, t_dur, t_depth, res
...
if __name__ == '__main__':
    ...
    if args.pipe == 'spoc':
        ...
        if args.lightkurve == True:
            ...
            p_best, f_best, p_fit, f_fit, e_fit, rp_best, a_best, inc_best, t0_best, t_dur, t_depth, res = lc_fit_spoc_lk(time, flux_flat, flux, err, per, epoch, tdur, tic)
    
        else: p_best, f_best, p_fit, f_fit, e_fit, rp_best, a_best, inc_best, t0_best, t_dur, t_depth, res = lc_fit_spoc(time, flux, err, per, epoch, tdur, tic, Tmag, Rs)
    
    elif args.pipe == 'qlp':
		
        time, flux, tic = tess_LC_dataload_qlp(args.fn)
        ...
        p_best, f_best, p_fit, f_fit, rp_best, a_best, inc_best, t0_best, t_dur, t_depth, res = lc_fit_qlp(time, flux, per, epoch, tdur, tic)
